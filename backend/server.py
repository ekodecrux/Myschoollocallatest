...
# Root-level health endpoint for Kubernetes (before any prefix)
@app.get("/health")
...
async def send_self_registration_welcome_email(to_email: str, name: str, role: str, school_name: str = "MySchool"):
    """Send welcome email for self-registered users"""
    subject = f"Welcome to {school_name}! Registration Successful"
    html_content = f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p>Dear <strong>{name}</strong>,</p>
                <p>Thank you for registering with us! Your account has been created successfully as a <strong>{role}</strong>.</p>
                ...
                <p><a href="https://portal.myschoolct.com" class="btn">Login Now</a></p>
...
async def login(request: UserLoginRequest):
    ...
    if user.get("school_code") and user.get("role") in [UserRole.TEACHER, UserRole.STUDENT]:
        school_admin = await db.users.find_one({
            "school_code": user["school_code"], 
            "role": UserRole.SCHOOL_ADMIN
        })
        if school_admin and school_admin.get("disabled", False):
            raise HTTPException(status_code=403, detail="Your school is currently disabled. Please contact administrator.")
...
async def register(
    ...
    # Send welcome email
    # For self-registered users: send confirmation email
    # For admin-created users: send password email
    school_name = "MySchool"
    if request.school_code:
        school = await db.schools.find_one({
            "code": {"$regex": f"^{re.escape(request.school_code)}$", "$options": "i"}
        })
        if school:
            school_name = school.get("name", "MySchool")
    
    if registration_type == "self_registered":
        # Self-registered user - send welcome/confirmation email
        background_tasks.add_task(
            send_self_registration_welcome_email,
            request.email,
            request.name,
            request.user_role,
            school_name
        ...
        # Admin-created user - send password email
        background_tasks.add_task(
            send_welcome_email,
            request.email,
            request.name,
...
async def update_user_by_id(
    ...
    """Update any user by ID - Admin/Super Admin only"""
    user_role = current_user.get("role")
    ...
    # School admins can only edit users in their school
    if user_role == UserRole.SCHOOL_ADMIN:
        if target_user.get("school_code") != current_user.get("school_code"):
            raise HTTPException(status_code=403, detail="You can only edit users in your school")
    
    # Allowed fields for update
    allowed_fields = ["name", "mobile_number", "address", "city", "state", "postal_code", 
                      "principal_name", "class_name", "section_name", "roll_number", "father_name", "parent_name"]
    
    update_data = {}
    for key, value in body.items():
        # Convert camelCase to snake_case
        snake_key = ''.join(['_' + c.lower() if c.isupper() else c for c in key]).lstrip('_')
        if snake_key in allowed_fields and value is not None:
            update_data[snake_key] = value
    
    # Also update school name if it's a school admin being updated
    if target_user.get("role") == UserRole.SCHOOL_ADMIN and "name" in update_data:
        # Update the school collection too
        await db.schools.update_one(
            {"code": target_user.get("school_code")},
...
async def list_users_by_role(
    ...
    # Get teacher and student counts for SCHOOL_ADMIN (schools)
    school_teacher_counts = {}
    school_student_counts = {}
    if role == "SCHOOL_ADMIN":
        school_codes_for_counts = [u.get("school_code") for u in users if u.get("school_code")]
        if school_codes_for_counts:
            ...
            for item in teacher_count_results:
                school_teacher_counts[item["_id"]] = item["count"]
            ...
            for item in student_count_results:
                school_student_counts[item["_id"]] = item["count"]
    
    # Transform to frontend expected format (camelCase)
    transformed_users = []
    for user in users:
        # Helper function to safely get string values (handle NaN/None)
        def safe_str(val):
        ...
        user_data = {
            ...
            "emailId": safe_str(user.get("email")),
            "name": safe_str(user.get("name")),
            "mobileNumber": safe_str(user.get("mobile_number")),
            "role": user.get("role"),
            "schoolCode": safe_str(user.get("school_code")),
            "schoolName": safe_str(user.get("school_name")) or school_names.get(user.get("school_code"), ""),
            "teacherCode": safe_str(user.get("teacher_code")),
            "teacherName": teacher_names.get(user.get("teacher_code"), "") if user.get("role") == "STUDENT" else "",
            "studentCode": safe_str(user.get("student_code")),
            "studentCount": teacher_student_counts.get(user.get("teacher_code"), 0) if user.get("role") == "TEACHER" else 0,
            "teachersEnrolled": school_teacher_counts.get(user.get("school_code"), 0) if user.get("role") == "SCHOOL_ADMIN" else 0,
            "studentsEnrolled": school_student_counts.get(user.get("school_code"), 0) if user.get("role") == "SCHOOL_ADMIN" else 0,
            "principalName": safe_str(user.get("principal_name")),
            "credits": user.get("credits", 0),
            "disabled": user.get("disabled", False),
            "className": safe_str(user.get("class_name")),
            "sectionName": safe_str(user.get("section_name")),
            "rollNumber": safe_str(user.get("roll_number")),
            "fatherName": safe_str(user.get("father_name")),
            "city": safe_str(user.get("city")),
            "state": safe_str(user.get("state")),
            "address": safe_str(user.get("address")),
            "postalCode": safe_str(user.get("postal_code")),
...
async def search_users(
    ...
    # Get school names and principal names for users
    school_codes = list(set([u.get("school_code") for u in users if u.get("school_code")]))
    school_names = {}
    school_principals = {}
    if school_codes:
        # Get school info from schools collection
        schools_info = await db.schools.find(
            {"code": {"$in": school_codes}},
            {"_id": 0, "code": 1, "name": 1, "principal_name": 1}
        ).to_list(len(school_codes))
        for school in schools_info:
            code = school.get("code")
            school_names[code] = school.get("name") or ""
            school_principals[code] = school.get("principal_name") or ""
        
        # Also check users collection for school_name (fallback)
        if not school_names:
            schools = await db.users.find(
                {"role": "SCHOOL_ADMIN", "school_code": {"$in": school_codes}},
                {"_id": 0, "school_code": 1, "name": 1, "school_name": 1}
            ...
            for school in schools:
                code = school.get("school_code")
                if code not in school_names or not school_names[code]:
                    school_names[code] = school.get("school_name") or school.get("name") or ""
    
    # Transform to frontend expected format
    results = []
    
    # Get teacher/student counts for SCHOOL_ADMIN users
    ...
    for user in users:
        user_data = {
            ...
            "principalName": user.get("principal_name") or school_principals.get(user.get("school_code"), ""),
            "city": user.get("city"),
        ...
        # Add teacher/student counts for schools
...
async def add_user(
    ...
    if request.user_role == UserRole.STUDENT:
        ...
        if user_role == UserRole.SUPER_ADMIN:
            ...
            # Validate school code exists (case-insensitive)
            school_code_upper = request.school_code.upper()
            school_exists = await db.users.find_one({
                "school_code": {"$regex": f"^{re.escape(request.school_code)}$", "$options": "i"},
                "role": UserRole.SCHOOL_ADMIN
            })
            if not school_exists:
                # Also check schools collection
                school_exists = await db.schools.find_one({
                    "code": {"$regex": f"^{re.escape(request.school_code)}$", "$options": "i"}
                })
            if not school_exists:
                raise HTTPException(status_code=400, detail=f"School with code '{request.school_code}' not found")
            # Use the actual school code from database for consistency
            if school_exists:
                request.school_code = school_exists.get("school_code") or school_exists.get("code")
            # Validate teacher code exists (case-insensitive)
            teacher_exists = await db.users.find_one({
                "teacher_code": {"$regex": f"^{re.escape(request.teacher_code)}$", "$options": "i"},
                "role": UserRole.TEACHER
            })
            if not teacher_exists:
                raise HTTPException(status_code=400, detail=f"Teacher with code '{request.teacher_code}' not found")
            # Use actual teacher code from database
            if teacher_exists:
                request.teacher_code = teacher_exists.get("teacher_code")
...
async def check_credits(current_user: dict = Depends(get_current_user)):
    ...
    # Determine current plan based on credits or subscription status
    current_plan = None
    if subscription_status == "enterprise" or credits >= 2000:
        current_plan = "enterprise"
    elif subscription_status == "premium" or credits >= 500:
        current_plan = "premium"
    elif subscription_status == "basic" or credits >= 100:
        current_plan = "basic"
...
@users_router.get("/subscription-history")
async def get_subscription_history(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for payment in payments:
            history.append({
                "plan_name": payment.get("plan_name", "Unknown Plan"),
        ...
        logging.error(f"Error fetching subscription history: {e}")
        return {"history": []}
...
async def use_credits(
    ...
    # Log the download activity with timestamp for analytics
    ...
    try:
        ...
        def is_valid_name(name):
            if not name or name.lower() == 'nan':
                return False
            # Allow only letters and spaces (no special characters or numbers)
...
async def bulk_upload_users(
    ...
    try:
        ...
        # Helper function to validate name (only letters and spaces allowed)
        def validate_name(name_value: str, field_name: str) -> tuple[bool, str]:
            if not name_value:
                return True, name_value  # Empty is OK
            # Only allow letters and spaces
            if not re.match(r'^[A-Za-z\s]+$', name_value):
                return False, f"{field_name} can only contain letters and spaces (no special characters or numbers)"
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Validate name if provided - only letters and spaces allowed
                if name:
                    is_valid, error_msg = validate_name(name, "Name")
                    if not is_valid:
                        results["failed"] += 1
...
async def bulk_upload_teachers(
    ...
    try:
        ...
        def validate_name(name_value: str, field_name: str) -> tuple[bool, str]:
            if not name_value:
                return True, name_value  # Empty is OK, will be caught by required field check
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Validate teacher name - only letters and spaces allowed
                is_valid, error_msg = validate_name(name, "Teacher Name")
                if not is_valid:
                    results["error_count"] += 1
                    results["errors"].append(f"Row {idx+2}: {error_msg}")
                    continue
                ...
                # Determine school code - use from CSV if provided, otherwise use user's school code (preserve original case)
                csv_school_code = str(row.get('school_code', '')).strip() if pd.notna(row.get('school_code')) else ''
                if user_role == UserRole.SUPER_ADMIN:
                ...
                # Verify school exists - check both schools collection and users with SCHOOL_ADMIN role (case-insensitive)
                school = await db.schools.find_one({
                    "code": {"$regex": f"^{re.escape(school_code)}$", "$options": "i"}
                })
                school_user = None
                actual_school_code = school_code  # Will be updated with actual code from DB
                if not school:
                    # Also check if school_code exists in users collection (for self-registered schools)
                    school_user = await db.users.find_one({
                        "school_code": {"$regex": f"^{re.escape(school_code)}$", "$options": "i"},
                        "role": "SCHOOL_ADMIN"
                    })
                    if not school_user:
                        results["error_count"] += 1
                        ...
                        continue
                    actual_school_code = school_user.get("school_code", school_code)
                else:
                    actual_school_code = school.get("code", school_code)
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                father_name = str(row.get('father_name', '')).strip() if row.get('father_name') else None
                ...
                is_valid, error_msg = validate_name(name, "Student Name")
                if not is_valid:
                    results["error_count"] += 1
                    results["errors"].append(f"Row {idx+2}: {error_msg}")
                    continue
                
                # Validate parent/father name if provided - only letters and spaces allowed
                if father_name:
                    is_valid, error_msg = validate_name(father_name, "Parent Name")
                    if not is_valid:
                        results["error_count"] += 1
                        results["errors"].append(f"Row {idx+2}: {error_msg}")
                        continue
...
async def global_search(
    ...
    # Normalize class-related queries (e.g., "3 rd class maths" -> "class-3", "class 3 maths" -> "class-3")
    normalized_query = original_query.lower()

    class_patterns = [
        (r'(\d+)\s*(?:st|nd|rd|th)?\s*class', r'class-\1'),  # "3rd class" or "3 rd class" -> "class-3"
        (r'class\s*-?\s*(\d+)', r'class-\1'),  # "class 3" or "class-3" -> "class-3"
    ]
    for pattern, replacement in class_patterns:
        normalized_query = re.sub(pattern, replacement, normalized_query)
    
    # Add normalized class term to search
    class_match = re.search(r'class-(\d+)', normalized_query)
    if class_match:
        class_num = class_match.group(1)
        search_terms.extend([f'class-{class_num}', f'class{class_num}', f'class {class_num}'])
    
    # Generate soundex codes for phonetic matching
    soundex_codes = [soundex(term) for term in search_terms if term]
...
async def fetch_images(body: dict, response: Response):
    ...
    if is_one_click_menu:
        ...
        if first_part in ["ONE_CLICK_RESOURCE_CENTRE", "ONE_CLICK_RESOURCE_CENTER", "SECTIONS"]:
            ...
            if len(parts) > 2:
                # Third part is sub_menu (NURSERY, PRIMARY, etc.)
                query["sub_menu"] = {"$regex": f"^{parts[2]}$", "$options": "i"}
...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    ...
    if user_role == UserRole.SUPER_ADMIN:
        ...
        # Get today's downloads from download_logs
        todays_downloads_pipeline = [
            {"$match": {"timestamp": {"$gte": today}}},
            {"$group": {"_id": None, "total": {"$sum": "$credits_used"}}}
        ]
        todays_result = await db.download_logs.aggregate(todays_downloads_pipeline).to_list(1)
        stats["todaysDownloads"] = todays_result[0]["total"] if todays_result else 0
        ...
        # Get today's downloads for this school
        todays_downloads_pipeline = [
            {"$match": {"school_code": school_code, "timestamp": {"$gte": today}}},
            {"$group": {"_id": None, "total": {"$sum": "$credits_used"}}}
        ]
        todays_result = await db.download_logs.aggregate(todays_downloads_pipeline).to_list(1)
        stats["todaysDownloads"] = todays_result[0]["total"] if todays_result else 0
...
async def get_user_logs_stats(current_user: dict = Depends(get_current_user)):
    ...
    # Total downloads (sum of credits_used) - historical total
    pipeline = [
        {"$match": query} if query else {"$match": {}},
        {"$group": {"_id": None, "total": {"$sum": "$credits_used"}}}
    ]
    download_result = await db.users.aggregate(pipeline).to_list(1)
    total_downloads_all_time = download_result[0]["total"] if download_result else 0
    
    # Today's downloads from download_logs collection
    download_query = {"timestamp": {"$gte": today}}
    if school_code and user_role != UserRole.SUPER_ADMIN:
        download_query["school_code"] = school_code
    
    todays_downloads_pipeline = [
        {"$match": download_query},
        {"$group": {"_id": None, "total": {"$sum": "$credits_used"}}}
    ]
    todays_result = await db.download_logs.aggregate(todays_downloads_pipeline).to_list(1)
    todays_downloads = todays_result[0]["total"] if todays_result else 0
    
    return {
        "todays_activity": todays_activity,
        "total_downloads": todays_downloads,  # This is now TODAY's downloads for the analytics card
        "total_downloads_all_time": total_downloads_all_time,  # Keep historical total if needed
...
async def create_razorpay_order(
    ...
    try:
        ...
        order_doc = {
            ...
            "plan_type": request.plan_type,
            "plan_name": plan.name,
            "amount": plan.price,  # Store in paise for consistency
            "credits": plan.credits,
            "currency": "INR",
            "status": "created",
        ...
        return {
            ...
            "amount": plan.price,  # In paise for Razorpay
            "currency": "INR",
            ...
            "user": {
                ...
                "phone": current_user.get("mobile", "")
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to create order: {str(e)}")

@payment_router.post("/razorpay/verify-payment")
async def verify_razorpay_payment(
    ...
    try:
        ...
        # Find the order
        order = await db.orders.find_one({"razorpay_order_id": request.razorpay_order_id})
        if not order:
        ...
        if payment["status"] == "captured":
            credits_to_add = order.get("credits", 0)
            plan_type = order.get("plan_type", "basic")
            plan_name = order.get("plan_name", "Basic Plan")
            amount = order.get("amount", 0)
            
            # Determine subscription status based on plan
            subscription_status = plan_type
            
            # Add credits and update subscription status
            await db.users.update_one(
                {"id": current_user["id"]},
                {
                    ...
                    "$set": {
                        ...
                        "last_subscription_plan": plan_name,
                        "last_subscription_date": datetime.now(timezone.utc).isoformat()
                    }
                }
            )
            ...
            return {
                "success": True,
                "message": f"Payment successful! {credits_to_add} credits added to your account.",
                "credits_added": credits_to_add,
                "plan": plan_name,
                "subscription_status": subscription_status
            }
...
@payment_router.get("/admin/subscription-stats")
async def get_subscription_stats(current_user: dict = Depends(require_super_admin)):
    """Get aggregated subscription and credits stats for Super Admin dashboard"""
    
    # Get all users onboarded by super admin (users without school_code or with registration_type='super_admin_onboarded')
    # Also get School Admins and their subscription status
    
    pipeline = [
        {
            "$match": {
                "role": {"$in": ["SCHOOL_ADMIN", "TEACHER", "STUDENT"]},
                "$or": [
                    {"added_by": current_user["id"]},
                    {"registration_type": "super_admin_onboarded"},
                    {"role": "SCHOOL_ADMIN"}  # All school admins
                ]
            }
        },
        {
            "$group": {
                ...
                "subscribed_users": {
                    "$sum": {
                        "$cond": [{"$gt": [{"$ifNull": ["$credits", 0]}, 0]}, 1, 0]
                    }
                }
            }
        }
    ...
    revenue_pipeline = [
        ...
        {"$group": {
            ...
            "total_orders": {"$sum": 1},
            "total_credits_sold": {"$sum": {"$ifNull": ["$credits", 0]}}
        }}
    ]
    revenue_stats = await db.orders.aggregate(revenue_pipeline).to_list(1)
    ...
    for stat in role_stats:
        ...
        stats_by_role[role] = {
            ...
            "subscribed_users": stat["subscribed_users"]
        }
    
    return {
        "role_stats": stats_by_role,
        "revenue": revenue_stats[0] if revenue_stats else {
            "total_revenue": 0,
            "total_orders": 0,
        ...
        "plans": [
            {"id": "basic", "price": 499, "credits": 100},
            {"id": "premium", "price": 999, "credits": 500},
            {"id": "enterprise", "price": 2499, "credits": 2000}
        ]
    }

@payment_router.get("/user/credits")
async def get_user_credits(current_user: dict = Depends(get_current_user)):
    """Get current user's credits - for real-time display"""
    user = await db.users.find_one({"id": current_user["id"]}, {"_id": 0, "credits": 1, "role": 1})
...
# Digital board
digital_board_router = APIRouter(prefix="/digital-boards", tags=["Digital Board"])

class DigitalBoardCreate(BaseModel):
    name: str
    data: str
    thumbnail: Optional[str] = None

class DigitalBoardUpdate(BaseModel):
    name: Optional[str] = None
    data: Optional[str] = None
    thumbnail: Optional[str] = None

@digital_board_router.get("")
async def get_digital_boards(
    current_user: dict = Depends(get_current_user)
):
    """Get all digital boards for the current teacher"""
    user_id = current_user["id"]
    
    boards = await db.digital_boards.find(
        {"user_id": user_id},
        {"_id": 0, "data": 0}  # Exclude large data field for list view
    ).sort("updated_at", -1).to_list(100)
    
    return {"boards": boards}

@digital_board_router.post("")
async def create_digital_board(
    board: DigitalBoardCreate,
    current_user: dict = Depends(get_current_user)
):
    ...
    await db.digital_boards.insert_one(board_doc)
    
    return {"board_id": board_id, "message": "Board created successfully"}

@digital_board_router.get("/{board_id}")
async def get_digital_board(
    ...
    board = await db.digital_boards.find_one(
...
@digital_board_router.put("/{board_id}")
async def update_digital_board(
    board_id: str,
    board: DigitalBoardUpdate,
    current_user: dict = Depends(get_current_user)
):
    """Update a digital board"""
    user_id = current_user["id"]
    
    update_data = {"updated_at": datetime.now(timezone.utc).isoformat()}
    if board.name is not None:
        update_data["name"] = board.name
    if board.data is not None:
        update_data["data"] = board.data
    if board.thumbnail is not None:
        update_data["thumbnail"] = board.thumbnail
    
    result = await db.digital_boards.update_one(
        {"id": board_id, "user_id": user_id},
        {"$set": update_data}
    )
    ...
    return {"board_id": board_id, "message": "Board updated successfully"}

@digital_board_router.delete("/{board_id}")
async def delete_digital_board(
    board_id: str,
    current_user: dict = Depends(get_current_user)
):
    ...
    result = await db.digital_boards.delete_one(
        {"id": board_id, "user_id": user_id}
    )
...
# Lesson plans
lesson_plan_router = APIRouter(prefix="/lesson-plans", tags=["Lesson Plans"])
...
class LessonPlanUpdate(BaseModel):
    ...
    homework: Optional[str] = None
    notes: Optional[str] = None
    use_digital_board: Optional[bool] = None
...
@lesson_plan_router.get("/today")
async def get_today_lesson_plans(
    current_user: dict = Depends(get_current_user)
):
    """Get today's lesson plans for the current teacher"""
    user_id = current_user["id"]
    ...
    return {"lesson_plans": plans, "date": today}
...
async def create_lesson_plan(
    ...
    plan_doc = {
        ...
        "notes": plan.notes,
        "use_digital_board": plan.use_digital_board,
        "created_at": now,
        "updated_at": now
    }
    ...
    return {"plan_id": plan_id, "message": "Lesson plan created successfully"}

@lesson_plan_router.get("/{plan_id}")
...
@lesson_plan_router.put("/{plan_id}")
async def update_lesson_plan(
    ...
    update_data = {"updated_at": datetime.now(timezone.utc).isoformat()}
    for field in ["title", "subject", "class_name", "date", "objectives", "content", 
                  "activities", "resources", "homework", "notes", "use_digital_board"]:
    ...
    return {"plan_id": plan_id, "message": "Lesson plan updated successfully"}

@lesson_plan_router.delete("/{plan_id}")
...
rest_router.include_router(digital_board_router)
rest_router.include_router(lesson_plan_router)
