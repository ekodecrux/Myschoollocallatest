...
async def get_user_details(current_user: dict = Depends(get_current_user)):
    ...
    # If user has school_code but no school_name, fetch it from schools collection
    if user.get("school_code") and not user.get("school_name"):
        school = await db.schools.find_one({"code": user["school_code"]}, {"_id": 0})
        if school:
    ...
    if school_code is not None:
        ...
        if school_code:
            ...
            if not school:
                ...
                if not school_admin:
                    raise HTTPException(status_code=400, detail=f"School code '{school_code}' not found in system. Please verify the school code or remove it from CSV to use your default school code.")
...
async def update_credits(
    ...
    if action == "add":
        ...
        new_credits = current_credits - credits
        if new_credits < 0:
            raise HTTPException(status_code=400, detail="Cannot remove more credits than user has")
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if not school:
                    results["failed"] += 1
                    results["errors"].append(f"Row {idx+2}: School code '{school_code}' not found in system. Please verify the school code or remove it from CSV to use your default school code.")
                    continue
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                csv_school_code = str(row.get('school_code', '')).strip().upper()
                if user_role == UserRole.SUPER_ADMIN:
                    ...
                    # For School Admin - use CSV school code if provided, otherwise use their own
                    school_code = csv_school_code if csv_school_code else user_school_code
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if user_role == UserRole.SUPER_ADMIN:
                    ...
                    # For School Admin/Teacher - use CSV school code if provided, otherwise use their own
                    school_code = csv_school_code if csv_school_code else user_school_code
                ...
                student = {
                    ...
                    "teacher_code": str(row.get('teacher_code', '')).strip() or None,
                    "address": str(row.get('address', '')).strip() or None,
                    "city": str(row.get('city', '')).strip() or None,
                    "state": str(row.get('state', '')).strip() or None,
                    "postal_code": str(row.get('postal_code', '')).strip() or None,
...
PDF_THUMBNAIL_CACHE = Path("/tmp/pdf_thumbnails")
...
async def get_pdf_thumbnail(
    ...
    try:
        ...
        # Fetch PDF from R2
        r2_base = os.environ.get("R2_BASE_URL", "https://pub-0ddb0004edaa4980a580d6c3f79b3a3f.r2.dev")
